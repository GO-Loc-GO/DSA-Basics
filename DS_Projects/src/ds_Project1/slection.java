...
public class slection {
	...
	public static void main(String[] args) {
		...
		//for testA
		selectionSort(testA);
		System.out.println("\ntestA after selection sort: {" + Arrays.toString(testA)+"}");
		...
		selectionSort(temp);   //sort the template array
		for(i=0;i<testB.length;i++) {
		...
		System.out.print("testB after selection sort: {");    //print the results of testB
		for(i=0;i<testB.length;i++) {
			System.out.print(Arrays.toString(testB[i]));
			if(i!=testB.length-1) {
				System.out.print(',');
