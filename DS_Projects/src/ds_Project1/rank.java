...
public class rank {
	...
	public static void rankSort(int f[]) {
		int i,j,rank;
	...
	public static void main(String[] args) {
		...
		//for testA
		rankSort(testA);
		System.out.println("\ntestA after rank sort: {" + Arrays.toString(testA)+"}");
		...
		rankSort(temp);   //sort the template array
		for(i=0;i<testB.length;i++) {
		...
		System.out.print("testB after rank sort: {");		//print the results of testB
		for(i=0;i<testB.length;i++) {
			System.out.print(Arrays.toString(testB[i]));
			if(i!=testB.length-1) {
				System.out.print(',');
